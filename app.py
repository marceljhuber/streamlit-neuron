...
''')
    ...
    st.sidebar.write('if you have trouble run python -m before the command')
    st.sidebar.code('''
$ streamlit --help
...
pip install streamlit-nightly --upgrade
    ''')
    st.sidebar.markdown(
        '<small>Learn more about [experimental features]('
        'https://docs.streamlit.io/library/advanced-features/prerelease#beta-and-experimental-features)</small>',
        unsafe_allow_html=True)

    st.sidebar.markdown('''<hr>''', unsafe_allow_html=True)
    st.sidebar.markdown(
        '''<small>[Cheat sheet v1.25.0](https://github.com/daniellewisDL/streamlit-cheat-sheet)  | Aug 2023 | [Daniel 
        Lewis](https://daniellewisdl.github.io/)</small>''',
        unsafe_allow_html=True)
...
# * optional kwarg unsafe_allow_html = True
    ...
    col1.code('''
# Just like jupyter notebook, displays table
...
# we also have 3 columns in this app
...
# you can switch between them in the window
...
''')
    ...
    col1.code('''
# Plot charts with inbuilt functions
>>> st.line_chart(data)
>>> st.area_chart(data)
>>> st.bar_chart(data)
>>> st.scatter_chart(data)
# or use your own charting library
# such as Altair, Bokeh, matplotlib, Plotly, Vega, etc.
>>> st.altair_chart(altair_object/container)
>>> st.pyplot(matlotlib_figure)
...
# can do something like this: if st.button('Hit me'):
st.button('Hit me')
# to edit data object (useful for (hyper)parameter tuning maybe)
...
st.text_area('Area for textual entry (multi-line)')
...
''')
    ...
    col2.markdown(
        '<small>Learn how to [build chat-based apps]('
        'https://docs.streamlit.io/knowledge-base/tutorials/build-conversational-apps)</small>',
        unsafe_allow_html=True)
